...
- (NSString *)settingsKey {
    ...
    return [@"AdvancedAutoClicker.v2." stringByAppendingString:bundleID];
}
...
- (void)saveSettings {
    ...
    [NSUserDefaults.standardUserDefaults setObject:saved forKey:[self settingsKey]];
}
...
- (UIView *)createMarkerForPoint:(ACPoint *)point {
    ...
    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(dragMarker:)];
    [marker addGestureRecognizer:pan];

    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectMarker:)];
...
- (void)dragMarker:(UIPanGestureRecognizer *)gesture {
    ...
    CGPoint newCenter = CGPointMake(marker.center.x + translation.x, marker.center.y + translation.y);
...
- (void)buildGlobalPanel {
    self.globalPanel = [[UIView alloc] initWithFrame:CGRectMake(18, 120, 270, 220)];
    ...
    UILabel *title = [self labelWithText:@"AutoClicker" frame:CGRectMake(16, 12, 180, 28) fontSize:19];
    title.font = [UIFont boldSystemFontOfSize:19];
    [self.globalPanel addSubview:title];

    UIButton *close = [self buttonWithTitle:@"×" frame:CGRectMake(220, 10, 36, 32) action:@selector(toggleGlobalPanel)];
    [self.globalPanel addSubview:close];

    [self.globalPanel addSubview:[self labelWithText:@"Initial wait" frame:CGRectMake(16, 55, 105, 32) fontSize:14]];
    self.initialDelayField = [self numberFieldWithFrame:CGRectMake(130, 55, 80, 32)];
    ...
    [self.globalPanel addSubview:[self labelWithText:@"Loops (0 = ∞)" frame:CGRectMake(16, 95, 105, 32) fontSize:14]];
    self.loopField = [self numberFieldWithFrame:CGRectMake(130, 95, 80, 32)];
    self.loopField.text = [NSString stringWithFormat:@"%ld", (long)self.loopCount];
    [self.globalPanel addSubview:self.loopField];

    UIButton *add = [self buttonWithTitle:@"+ Add point" frame:CGRectMake(16, 145, 112, 42) action:@selector(addPoint)];
    [self.globalPanel addSubview:add];

    UIButton *hide = [self buttonWithTitle:@"Hide panel" frame:CGRectMake(140, 145, 112, 42) action:@selector(toggleGlobalPanel)];
    [self.globalPanel addSubview:hide];

    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(dragGlobalPanel:)];
...
- (void)buildPointPanel {
    ...
    self.pointTitle = [self labelWithText:@"Point" frame:CGRectMake(16, 12, 140, 28) fontSize:18];
    self.pointTitle.font = [UIFont boldSystemFontOfSize:18];
    [self.pointPanel addSubview:self.pointTitle];

    UIButton *close = [self buttonWithTitle:@"×" frame:CGRectMake(198, 10, 36, 32) action:@selector(hidePointPanel)];
    [self.pointPanel addSubview:close];

    [self.pointPanel addSubview:[self labelWithText:@"Wait after tap" frame:CGRectMake(16, 55, 112, 32) fontSize:14]];
    self.pointDelayField = [self numberFieldWithFrame:CGRectMake(142, 55, 80, 32)];
    [self.pointPanel addSubview:self.pointDelayField];

    [self.pointPanel addSubview:[self labelWithText:@"Hold duration" frame:CGRectMake(16, 95, 112, 32) fontSize:14]];
    self.pointHoldField = [self numberFieldWithFrame:CGRectMake(142, 95, 80, 32)];
    [self.pointPanel addSubview:self.pointHoldField];

    [self.pointPanel addSubview:[self labelWithText:@"Repeat count" frame:CGRectMake(16, 135, 112, 32) fontSize:14]];
    ...
    UIButton *delete = [self buttonWithTitle:@"Delete point" frame:CGRectMake(16, 190, 106, 42) action:@selector(deleteSelectedPoint)];
    delete.backgroundColor = [[UIColor systemRedColor] colorWithAlphaComponent:0.30];
    [self.pointPanel addSubview:delete];

    UIButton *done = [self buttonWithTitle:@"Done" frame:CGRectMake(132, 190, 102, 42) action:@selector(hidePointPanel)];
...
- (void)showPointPanelForPoint:(ACPoint *)point {
    ...
    y = MAX(8.0, MIN(screen.height - self.pointPanel.bounds.size.height - 8.0, y));

    self.pointPanel.frame = CGRectMake(x, y, self.pointPanel.bounds.size.width, self.pointPanel.bounds.size.height);
...
- (void)toggleGlobalPanel {
    [self commitFields];
    self.globalPanel.hidden = !self.globalPanel.hidden;

    if (!self.globalPanel.hidden) {
        self.initialDelayField.text = [NSString stringWithFormat:@"%.2f", self.initialDelay];
        self.loopField.text = [NSString stringWithFormat:@"%ld", (long)self.loopCount];
    }
...
- (void)install {
    ...
    UIPanGestureRecognizer *barPan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(dragBar:)];
    [self.bar addGestureRecognizer:barPan];

    UIButton *settings = [self buttonWithTitle:@"⚙︎" frame:CGRectMake(5, 7, 44, 44) action:@selector(toggleGlobalPanel)];
    settings.titleLabel.font = [UIFont systemFontOfSize:22];
    [self.bar addSubview:settings];

    self.runButton = [self buttonWithTitle:@"▶︎" frame:CGRectMake(5, 62, 44, 44) action:@selector(toggleRun)];
